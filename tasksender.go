...
func (rebuilder *Rebuilder) SendTasks(ctx context.Context) {
	...
	for {
		...
		for {
			...
			node = ele.(*Node)
			if node.Rebuilding > 1 || node.Status != 1 || node.Valid == 0 || node.Weight < float64(rebuilder.Params.WeightThreshold) || node.AssignedSpace <= 0 || node.Quota <= 0 || node.Version < int32(rebuilder.Params.MinerVersionThreshold) || time.Now().Unix()-node.Timestamp > 180 {
				continue
			}
			break
		}
		...
		pool.JobQueue <- func() {
			...
			if e != nil {
				entry.WithField(MinerID, node.ID).WithError(err).Errorf("Send rebuild task failed: %d--%s", e.Code, e.Msg)
			} else {
				entry.WithField(MinerID, node.ID).Infof("Send rebuild task OK,count %d", len(task.Tasklist))
			}
